...
static void handle_func_call(Buffer *b, Node *node) {
...
static void handle_func(Buffer *b, Node *node) {
	...
	buf_printf(b, ")");
	do_node2s(b, node->body);  // Handle the function body
}

// Function to handle compound statements
static void handle_compound_stmt(Buffer *b, Node *node) {
	buf_printf(b, "{");
...
// Function to handle a literals value (ast)
static void handle_ast_iterals(Buffer *b, Node *node) {
	switch (node->ty->kind) {
		case KIND_CHAR:
			switch (node->ival) {
				case '\n': buf_printf(b, "'\n'");   break;
...
//
// TODO:
// Implement function to handle Handle Arithmetic shift right and assign (>>=)
// for the OP_A_SAR case. And implement function to handle Right Arithmetic 
// Shift Operations for the OP_SAR case
static void do_node2s(Buffer *b, Node *node) {
	if (!node) {
		buf_printf(b, "(nil)");
		return;
