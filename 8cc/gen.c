...
//
// Constructs and returns a string that lists the names of functions
// in the "functions" vector, with each function name separated by 
// " -> ". The string is dynamically built using a Buffer.
static char *get_caller_list() {
	// Create a new buffer to accumulate the function names
	// if memory allocation fails, return null
	Buffer *b = make_buffer();
	if (b == NULL) return NULL;

	// Iterate through the vector of function names
	for (int i = 0; i < vec_len(functions); i++) {
		// If not the first function, add a separator " -> " between 
		// function names, then Add " -> " separator
		if (i > 0) buf_printf(b, " -> ");

		// Append the function name to the buffer
		buf_printf(b, "%s", vec_get(functions, i));
	}

	// Write a null terminator at the end of the buffer to 
	// mark the string's end, before returning the pointer 
	// to the buffer's internal string representation
	buf_write(b, '\0');
	return buf_body(b);
}
...
static void emit_lload(Type *ty, char *base, int off) {
	...
	switch (ty->kind) {
		...
		case KIND_FLOAT:
			assert_float();
			break;
		case KIND_DOUBLE: case KIND_LDOUBLE:
			assert_float();
			break;
		default:
			emit("mov B, %s", base);
...
static void emit_pointer_arith(char kind, Node *left, Node *right) {
	...
	if (left->ty->ptr->size == 2) emit("add A, A");
	if (left->ty->ptr->size > 2)  emit("mul A, %d", left->ty->ptr->size);

	emit("mov B, A");
	pop("A");

	if (kind == '+')       emit("add A, B");
	else if (kind == '-')  emit("sub A, B");
	else                   error("invalid operator '%d'", kind);
...
static void emit_store(Node *var) {
	...
	switch (var->kind) {
		case AST_DEREF:
			...
			break;
		case AST_STRUCT_REF:
			emit_assign_struct_ref(var->struc, var->ty, 0);
			break;
		case AST_LVAR:
...
static void emit_binop_int_arith(Node *node) {
	...
	switch (node->kind) {
		...
		case '%':
			...
			emit("%smod A, B", sn);
			break;
		case OP_SAL: emit("shl A, B"); break;
		case OP_SAR:
...
void emit_call_builtin(char *fname) {
	char *end = make_label();
	emit("mov A, %s", end);
	push("A");
	emit("jmp %s", fname);
	...
	switch (node->kind) {
		...
		case AST_FUNCALL:
			if (maybe_emit_builtin(node))
				return;
			// fall through (what do you mean by this???)
		...
		case AST_COMPOUND_STMT:
			emit_compound_stmt(node); return;
		case AST_STRUCT_REF:
...
static void handle_unknown_ptr(Node *val) {
	Node *base = NULL;
...
//
// emit_data_primtype -  Generates assembly representation for primitive types
//
static void emit_data_primtype(Type *ty, Node *val, int depth) {
	...
	switch (ty->kind) {
		...
		case KIND_BOOL:
			// Emit a byte for boolean values (0 or 1)
			emit(".byte %d", !!eval_intexpr(val, NULL));
			break;
		case KIND_CHAR:
			// Emit a byte for char values
			emit(".byte %d", eval_intexpr(val, NULL));
			break;
		case KIND_SHORT:
			// Emit a short for short integer values
			emit(".short %d", eval_intexpr(val, NULL));
			break;
		case KIND_INT:
			// Emit an int for integer values
			emit(".int %d", eval_intexpr(val, NULL));
			break;
		case KIND_LONG:
		case KIND_LLONG:
			// Emit a long for long integer values
		...
		case KIND_PTR:
			...
			// is the kind a char pointer?
			if (val->operand->ty->kind == KIND_ARRAY && val->operand->ty->ptr->kind == KIND_CHAR) {
				emit_data_charptr(val->operand->sval, depth);
			} else if (val->kind == AST_GVAR) {
				emit(".ptr %s", val->glabel);
			} else {
				handle_unknown_ptr(val);
			}
			break;
		default:
...
static void emit_bss(Node *v) {
	...
	emit("%s:\n", v->declvar->glabel);
	for (int i = 0; i < v->declvar->ty->size && v->declvar->ty->size != -1; i++) {
		emit(".byte 0");
	}
}
...
static void emit_func_prologue(Node *func) {
	...
	off &= -8; // Enusre that the stack is aligned to an 8-byte boundary

	// Allocate stack space for local variables if necessary
	int localarea = -off; // Calculate the total local area size
	if (localarea) {
		emit("sub SP, %d", localarea); // Adjust stack pointer
		stackpos += localarea;         // Update the current stack position
	}
}
